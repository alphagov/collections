...
class BrexitLandingPageTest < ActionDispatch::IntegrationTest
  ...
  context "buckets" do
    it "variant A renders buckets sections" do
      given_there_is_a_brexit_taxon
      when_i_visit_the_brexit_landing_page_with_dynamic_list("A")
      then_i_can_see_the_buckets_section_variant_a
    end

    it "variant B renders buckets sections" do
      given_there_is_a_brexit_taxon
      when_i_visit_the_brexit_landing_page_with_dynamic_list("B")
      then_i_can_see_the_buckets_section_variant_b
    end
  end
  ...
  %w(A B).each do |variant|
    context "when there should be a dynamic list" do
      it "renders the brexit page" do
        given_there_is_a_brexit_taxon
        when_i_visit_the_brexit_landing_page_with_dynamic_list(variant)
    ...
    context "when there should not be a dynamic list" do
      it "does not show the list" do
        given_there_is_a_brexit_taxon
        when_i_visit_the_brexit_landing_page_without_dynamic_list(variant)
